...
To opt in to instant docker-updater notifications after a push (recommended for private images, which don't emit GitHub package webhooks), add the url input and secret. The simplest approach is an org-level variable and secret:
...
jobs:
  publish-ghcr:
    uses: wow-look-at-my/actions/.github/workflows/publish-ghcr.yml@master
    with:
      updater-webhook-url: ${{ vars.DOCKER_UPDATER_WEBHOOK_URL }}
    secrets:
