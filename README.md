...
2. **Install** — On cache miss, `apt-get download ripgrep` fetches the `.deb` (~1 MB, no install, no postinst scripts). Then every run does three things. `dpkg-deb -x` unpacks the `.deb` into a temp dir. Next, `usr/bin/rg` is copied to `~/.local/bin/rg`. Finally, `~/.local/bin` is added to `GITHUB_PATH`, so later steps can call `rg` directly.
