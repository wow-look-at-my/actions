...
- `push-excludes-tags/` - Node.js action (fails CI when a workflow's `push` trigger names no ref filter. A `push:` carrying no `branches`, `branches-ignore`, `tags`, or `tags-ignore` matches every ref, so the tag a release pushes starts the whole workflow again on the commit that just passed it. Naming `tags:` passes, because that workflow asks for tag pushes on purpose)
