...
- `secret-server/` - Composite action. Fetches secrets from secrets.pazer.io over OIDC and exports each as a masked environment variable. The export loop lives in `export-secrets.sh`, a separate file so its `dats/` suite can drive it directly. jq.exe on Windows writes CRLF, so a key or value read off its stdout is stripped of a trailing `\r` before it becomes a variable name
