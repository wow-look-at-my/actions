...
- `common-checks/` - Composite action. It is the one step that runs the checks every repo in the org wants, named in its `action.yml`. Composite is the shape it has to be: a node action cannot `uses:` another action, and GitHub composes actions only through composite steps. It scans the calling repo only: `.gitmodules` names paths that belong to another repository with its own CI, so those go into the `exclude` input. `run-once` gates the whole thing, so the scan happens once per workflow run rather than once per job
...
- `has-permission/` - Composite action. It reports whether a permission is granted to the running job. It reads the running workflow file and resolves one scope the way GitHub does: the job's own `permissions:` block, then the workflow block when the job declares none. A job block REPLACES the workflow block rather than merging with it. It never calls an API and never exercises the permission. `assert: true` fails the step instead of warning
...
- `no-tests-in-yaml/` - Node.js action (fails CI when a `run:` script holds a test rather than invoking the repository's own suite. Three rules read only `run:` scripts. The first is a redirect or heredoc writing a file that is a test by name (`*_test.go`, `*.test.ts`, `*.dats`, ...). The second is a comparison paired with a failure on one line (`grep -q … || exit 1`, `if ! grep …`, `[ … ] || { … }`, and the annotate-and-exit line a `case` arm uses). The third is a shell function named `assert*`/`expect*`/`must*`. A step that merely runs a command fails on its own exit code and matches nothing. It scans the whole local call chain, like `yaml-comment-block`, and an empty scan warns rather than passing. No input turns a rule off: a settable rule removes the rule, and a check that can be routed around earns a stricter replacement)
...
- `secret-server/` - Composite action. It fetches secrets from secrets.pazer.io over OIDC and exports each as a masked environment variable. The export loop lives in `export-secrets.sh`. A separate file lets its `dats/` suite drive it directly. jq.exe on Windows writes CRLF line endings. The script strips a trailing `\r` from each key and value before export
- `run-once/` - Node.js action. It claims a workflow run for one job, so the work behind the claim runs once per run. The mutex is `CreateCacheEntry` refusing a key that exists: `run-once-<run_id>-<run_attempt>-<name>`. The winner uploads a few bytes and finalizes, so a later job sees a stored entry rather than a reservation in flight. Every cache-service failure returns `first: true` with a warning, because work skipped in every job reports nothing. Only a refusal with an entry to show for it returns false. It is the only action here built with `ts0` (`ts0.json`, and a justfile that fetches `ts0.cjs` from buildhost). `packages: bundle` is set, because a node action ships no `node_modules`
- `push-excludes-tags/` - Node.js action (fails CI when a workflow's `push` trigger names no ref filter. A `push:` carrying no `branches`, `branches-ignore`, `tags`, or `tags-ignore` matches every ref. The tag a release pushes then starts the whole workflow again on the commit that just passed it. Naming `tags:` passes, because that workflow asks for tag pushes on purpose)
- `smart-cache/` - Node.js action (TypeScript compiled to JS)
- `ste-lint/` - Node.js action (the mechanical subset of ASD-STE100 Simplified Technical English over prose files. Sentence length, contractions, banned modal verbs, semicolons, comma splices, and hard-wrapped paragraphs FAIL. A paragraph is ONE line. A wrap is one author's guess at one reader's window, frozen into the file. Every later edit reflows the block. A two-word change then looks like a rewrite in the diff. That last rule is a house rule and claims no ASD rule number — the mapping doc says so rather than dressing it up as STE. YAML frontmatter (a leading `---`...`---` block) is exempt from it, the same as a fenced code block: it is data, not prose. Every rule reads a paragraph with its wrapped lines REJOINED (`src/blocks.ts`). Reading one physical line at a time cannot see a sentence, which left the length cap unenforceable on any hard-wrapped document. A finding still names the line its sentence starts on. A code span or a quotation is replaced by a same-length blank that opens with one letter. It counts as one word and can still open a sentence. A quotation ends at the next quotation mark or at the next blank line. Pairing across a whole document put every later span at the wrong place. An appositive list is a finding, because a reader cannot tell a renamed noun from the next item. `fixtures/` keeps the prose that broke each of these rules, and `src/fixtures.test.ts` fails on a fixture whose header names no rule. Passing file arguments runs it as a local CLI. It reports its own `uses:` ref, and it FAILS on a step carrying `continue-on-error` (`src/guard.ts`). Neither word cap is settable ABOVE 25 (`src/inputs.ts`): rule 6.3 sets that number, so a larger one removes the rule instead of configuring it. The heuristics only warn. They are passive voice, noun clusters, complex verb tenses, paragraph length, and dictionary word choice (`src/ste100-banned-words.ts`, extracted from the free PDF of ASD). A heuristic that fails a build teaches people to route around the check. See `docs/ste-lint-spec-mapping.md` for the full rule-by-rule mapping)
- `cache-size/` - Node.js action (TypeScript compiled to JS)
- `tag-runner/` - Node.js action (TypeScript compiled to JS)
- `tag-cleanup/` - Node.js action (deletes orphan-release tags whose action, branch, or version no longer exists. Existence is judged against the default branch tree, never the checkout, so a feature branch cannot sweep tags on its own run)
...
- `typescript/` - Node.js action. It runs an inline or file-based TypeScript script, tsc-validated, with injected helper globals. An inline `script:` carrying two or more consecutive `//`-only lines fails the step. A `file:` input is exempt. It also reads the running workflow and FAILS a step that runs it with no `name:` (`src/step-name.ts`). An unnamed step logs as the action, so every one of them reads `Run wow-look-at-my/actions@typescript#latest` and says nothing about the script. A step reached through a composite action is not in the workflow file, so that case reports nothing.
...
`dats/` holds this repository's own suite — the assertions release.yml used to spell out in bash. `dats/readme.dats` regenerates the README and diffs it, which is what keeps `generate-readme.sh` and the committed file in step. The `checks` job only invokes the suite.
