...
- **`dist/index.js` stays self-contained.** The bundle inlines the shared modules. The orphan release tag therefore ships exactly what it always did, and nothing needs `shared/` at runtime. The build recipe deletes `.tsc-out/`, so it can never end up in a tag.
