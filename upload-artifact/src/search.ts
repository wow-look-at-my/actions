// Ported from actions/upload-artifact v4.6.2 src/shared/search.ts
// (MIT License, Copyright GitHub, Inc. and contributors), adapted to this
// repo's house style. Glob semantics, LCA root selection, and log messages
// are kept upstream-equivalent so call sites behave identically.
...
/**
 * If multiple paths are specified, the least common ancestor (LCA) of the
 * search paths is used as the delimiter to control the directory structure
 * for the artifact. This function returns the LCA when given an array of
 * search paths.
 *
...
export async function findFilesToUpload(
	...
	const globber = await glob.create(searchPath, getDefaultGlobOptions(includeHiddenFiles || false));
	const rawSearchResults: string[] = await globber.glob();

	/*
		Files are saved with case insensitivity. Uploading both a.txt and A.txt will
		cause files to be overwritten. Detect any files that could be overwritten for
		user awareness.
	*/
	const set = new Set<string>();

	/*
		Directories will be rejected if attempted to be uploaded. This includes just
		empty directories so filter any directories out from the raw search results.
	*/
	...
	/*
		Special case for a single file artifact that is uploaded without a directory or wildcard pattern.
		The directory structure is not preserved and the root directory will be the single file's parent
		directory.
	*/
