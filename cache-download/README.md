...
A missing hand-off fails loudly: `fail-if-missing` defaults to `true`, so the job aborts instead of silently continuing without its files. Set it to `'false'` for an optional hand-off and check the `cache-matched-key` output.
...
| `fail-if-missing` | No | `true` | Fail the job if the hand-off is not found |
