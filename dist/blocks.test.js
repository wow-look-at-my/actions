...
(0, node_test_1.test)('YAML frontmatter is skipped whole, not read as an unjoined wrap', () => {
    const b = (0, blocks_1.blocks)(lines('---\nifenv: SOME_FLAG\n---\n# Title\n\nBody text here.'));
    strict_1.default.deepEqual(b.map((x) => x.text), ['Body text here.']);
    strict_1.default.equal((0, lint_1.lintText)('a.md', '---\nifenv: SOME_FLAG\n---\n\nBody text here.').wrappedLines.length, 0);
});
(0, node_test_1.test)('a --- that is not frontmatter, because it does not open the file, is left alone', () => {
    ...
    const b = (0, blocks_1.blocks)(lines('Body first.\n\n---\n\nMore body.'));
    strict_1.default.deepEqual(b.map((x) => x.text), ['Body first.', '---', 'More body.']);
});
