...
The builder also has two paren-free terminal shortcuts that run the command and resolve straight to a stdout-derived value: `.json<T>()` (returns `Promise<T>` — `await $\`...\`.json()`) and `.text()` (returns `Promise<string>`, single trailing newline trimmed). They exist because `await` binds looser than `.`/`()`, so `await $\`x\`.stdout.json()` parses as `await ($\`x\`.stdout.json())` and hits `.stdout` on the un-awaited builder (the `(await fetch()).json()` gotcha); the shortcuts sidestep it for the common stdout case and compose with the chained modifiers (`await $\`x\`.env({...}).json()`). The `.stdout.json()` / `.stderr.json()` forms on the resolved `ProcessOutput` still work for stderr or when you need the full result.
