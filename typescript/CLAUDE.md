...
- `test/action.yml`'s inline `outcome-gate` steps — proves the Jenkins-style "keep the job running, fail it at the end" pattern for callers: a `continue-on-error: true` step's `outcome` still reports `failure` even though `continue-on-error` masks its `conclusion` to `success`, so a later gate step can read `steps.<id>.outcome` and call `core.setFailed()` to fail the job without aborting it early. Checked with `continue-on-error: true` + a `test "${{ steps.outcome-gate.outcome }}" = "failure"` assertion, the same proof-of-failure shape `release.yml`'s `test-no-all-builds-job` job uses.
