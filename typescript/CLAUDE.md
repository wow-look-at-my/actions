...
- `src/comments.ts` — Comment-block gate for inline scripts: finds runs of two or more consecutive `//`-only lines with the TypeScript scanner (so `//` inside a string, template literal or regex never counts) and reports each run's 1-based line span
...
- The step log has exactly two groups: `Compiling script` (highlighted source echo, then `Type-check passed.` and `Transpiled output: N bytes`) and `Executing script`. The compile steps had a group each until their output was two lines total — three collapsibles to expand instead of one. The source is echoed FIRST inside the group so a throw in the transform still leaves the script in the log, and type diagnostics land in the same group as the source they point at. Note that the runner separately echoes an inline `script:` input in its own step-input dump before the action starts; that copy is the runner's and cannot be suppressed, formatted, or merged into (a `file:` input is the only way to shorten it).
- An INLINE `script:` carrying two or more consecutive comment-only lines fails the step (`findCommentBlocks`), with one `core.error` per block naming its line span, checked on the RAW script so the reported lines are user lines with no line map involved. The failure is DEFERRED: the comment errors are logged up front, but the type-check and the script itself still run to completion, and `core.setFailed()` only fires at the very end — so a caller sees the type-check result and the script's real output in the same run instead of one error per re-run. A type-check or runtime error is a different failure mode and still stops the step immediately, same as always. A `file:` input is exempt (`readUserScript` returns `inline`) — it is ordinary checked-in source, commented like any other file in its repo; the gate is aimed at prose pasted into a workflow YAML. Only `//` is covered; a trailing comment after code, a lone comment line, and comment lines split by code or a blank line are all fine. No opt-out input for an inline script.
...
- `test/action.yml`'s `outcome-gate` steps — proves the Jenkins-style "keep the job running, fail it at the end" pattern for callers: a `continue-on-error: true` step's `outcome` still reports `failure` even though `continue-on-error` masks its `conclusion` to `success`, so a later gate step can key its own `if:` off `steps.<id>.outcome` directly (no intermediate env var needed) and `exit 1` to fail the job without aborting it early. Checked with `continue-on-error: true` + a `test "${{ steps.outcome-gate.outcome }}" = "failure"` assertion, the same proof-of-failure shape `release.yml`'s `test-no-all-builds-job` job uses.
